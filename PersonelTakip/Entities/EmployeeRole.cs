...
namespace PersonelTakip.Entities
{
    public class EmployeeRole
    {
        ...
        {
            string updateQuery = @"UPDATE [dbo].[EmployeeRoles] SET
                                                                    [Status] = 'False'
                                                                    ,[EndDate] = getdate()
        ...
        private static List<EmployeeRole> ConvertDataTableToList(DataTable tablo)
        {
            ...
            for (int i = 0; i < tablo.Rows.Count; i++)
            {
                ...
                employeeRole.Id = Convert.ToInt32(tablo.Rows[i]["Id"]);
                employeeRole.EmployeeId = Convert.ToInt32(tablo.Rows[i]["EmployeeId"]);
                employeeRole.RoleId = Convert.ToInt32(tablo.Rows[i]["RoleId"]);
